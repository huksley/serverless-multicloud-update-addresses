...
const googleMapsApiLimit = promiseLimit(5)
...
async function updateAddresses(context: Context) {
  // AWS Lambda env does not provide context.log, fix it here
  if (!context.log) {
      ...
      return request
        ...
        .then(result => {
          ...
          let promises = recordset.map(async r => {
            ...
            return googleMapsApiLimit(() => googleMapsClient
              .geocode({
                address: addr
              })
              .asPromise())
              .then(rr => {
                ...
                if (
                  ...
                  return request
                    ...
                    .catch(e => { context.log("Failed to update table", e); failed ++ })
                } else {
                  context.log("Incomplete response", geores)
                  return 0
                }
              })
              .catch(err => { context.log("Failed to geocode", err); failed ++ })
          })
          ...
          return Promise.all(promises)
            .then(_ => {
              context.res = {
                ...
                body: "Updated " + n + " addresses, failed " + failed + " addresses"
              }

              ignoreExceptions(_ => sql.close())
              context.log("Finished updating addresses, updated:", n, "failed:", failed)
              return context.res
            })
            .catch(e => {
